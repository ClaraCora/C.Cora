...
typedef union {
    ...
    uint64_t alignment;
} cora_dns_sockaddr_array;

typedef struct {
    cora_dns_char_ptr domain;
    int32_t n_nameserver;
    cora_dns_sockaddr_array nameserver;
    uint16_t port;
    int32_t n_search;
    cora_dns_char_ptr search;
    int32_t n_sortaddr;
    cora_dns_char_ptr sortaddr;
    cora_dns_char_ptr options;
    uint32_t timeout;
...
typedef union {
    cora_dns_resolver **value;
    uint64_t alignment;
} cora_dns_resolver_array;

typedef struct {
    int32_t n_resolver;
    cora_dns_resolver_array resolver;
    int32_t n_scoped_resolver;
    cora_dns_resolver_array scoped_resolver;
} cora_dns_config;
#pragma pack(pop)

_Static_assert(offsetof(cora_dns_resolver, if_index) == 64,
               "unexpected dns_resolver prefix layout");
_Static_assert(offsetof(cora_dns_config, scoped_resolver) == 16,
               "unexpected dns_config prefix layout");

typedef cora_dns_config *(*cora_dns_configuration_copy_fn)(void);
typedef void (*cora_dns_configuration_free_fn)(cora_dns_config *);

static int cora_write_sockaddr(const struct sockaddr *address,
...
static int cora_contains_address(const char *out, int stride, int count,
...
static int cora_copy_resolver_servers(cora_dns_resolver **resolvers,
                                         int resolverCount,
    ...
    for (int i = 0; i < resolverCount && written < maxCount; i++) {
        cora_dns_resolver *resolver = NULL;
        ...
        for (int j = 0; j < resolver->n_nameserver && written < maxCount; j++) {
            ...
            if (!cora_write_sockaddr(address, interfaceIndex,
                                        candidate, (int)sizeof(candidate)) ||
                cora_contains_address(out, stride, written, candidate)) {
                continue;
            }
...
int cora_copy_system_dns(char *out, int stride, int maxCount) {
...
int cora_copy_scoped_dns(const char *interfaceName,
                            char *out, int stride, int maxCount) {
    ...
    cora_dns_configuration_copy_fn copyConfiguration =
        (cora_dns_configuration_copy_fn)dlsym(RTLD_DEFAULT, "dns_configuration_copy");
    cora_dns_configuration_free_fn freeConfiguration =
        (cora_dns_configuration_free_fn)dlsym(RTLD_DEFAULT, "dns_configuration_free");
    if (copyConfiguration == NULL || freeConfiguration == NULL) return 0;

    cora_dns_config *configuration = copyConfiguration();
    if (configuration == NULL) return 0;

    int written = cora_copy_resolver_servers(
        configuration->scoped_resolver.value, configuration->n_scoped_resolver,
        interfaceIndex, out, stride, maxCount, 0);
    if (written == 0) {
        written = cora_copy_resolver_servers(
...
static int cora_is_usable_interface_address(const struct sockaddr *address) {
...
int cora_copy_interface_addresses(const char *interfaceName,
                                     char *out, int stride, int maxCount) {
    if (interfaceName == NULL || interfaceName[0] == '\0' || out == NULL ||
        stride <= 0 || maxCount <= 0) return 0;
    ...
    for (struct ifaddrs *entry = interfaces;
        ...
        if (entry->ifa_name == NULL ||
            ...
            !cora_is_usable_interface_address(entry->ifa_addr)) {
            continue;
        }

        char candidate[128] = {0};
        if (!cora_write_sockaddr(entry->ifa_addr, interfaceIndex,
                                    candidate, (int)sizeof(candidate)) ||
            cora_contains_address(out, stride, written, candidate)) {
            continue;
        }
