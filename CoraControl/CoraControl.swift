...
@main
struct CoraControlBundle: WidgetBundle {
...
struct VPNStatusProvider: ControlValueProvider {
    ...
    func currentValue() async throws -> Bool {
        if let mgr = try? await coraManager() {
...
struct ToggleVPNIntent: SetValueIntent {
    static let title: LocalizedStringResource = "切换 Cora 代理"

    @Parameter(title: "开启")
    var value: Bool

    func perform() async throws -> some IntentResult {
        guard let mgr = try await coraManager() else {
...
enum VPNControlError: Error, CustomLocalizedStringResourceConvertible {
    ...
    var localizedStringResource: LocalizedStringResource {
        switch self {
        case .notConfigured: return "请先打开 Cora 连接一次，授权 VPN 配置后再用控制中心开关"
        }
    }
}
