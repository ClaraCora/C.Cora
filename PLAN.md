...
- 总览累计上下行可进入节点流量 TOP5；记录页同步展示节点排行，并支持按
  总计、下行、上行切换及进入该节点的连接记录。
- 记录页支持活动连接详情、结束连接、策略/节点/主机流量聚合、日志切换。
- 连接详情由 Network Extension 写入 App Group SQLite；单次 VPN 会话最多 20,000 条、
  最近 7 天且数据库不超过约 50 MB，App 被划掉后 NE 仍可继续写入。
- 节点流量使用独立的受限 SQLite 累计表，不随详细连接裁剪而回退；最多保留
  4,096 个节点桶，VPN 断开时与本次会话记录一并清空。
