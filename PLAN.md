...
- 记录页支持活动连接详情、结束连接、策略/主机流量聚合、节点流量 TOP5 和日志切换。
...
- 节点流量排行只在 App 侧按需扫描已有 SQLite 历史的 `chains/upload/download`
  字段，一次生成总计、上行和下行三个 TOP5 并缓存；NE 不维护节点 Map、累计表、
  触发器或新的 IPC 数据。App 被划掉期间 NE 继续按原有连接历史流程记录，App
  下次打开时补算完整排行。
- NE 控制面异步请求采用两个并发槽位，并绑定 VPN 会话代际；停止或重连后尚未
  开始的旧请求会直接丢弃，避免测速、Provider 刷新和大响应在队列中堆积。
